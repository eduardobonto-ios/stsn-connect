...
/**
 * Expected top-level sidebar modules per role, expressed as label regexes.
 * `present` = must be visible; `absent` = must NOT be visible.
 *
 * Derived from ROLE_PERMISSIONS + getNavItemsForRole. Assertions read the
 * RENDERED sidebar (not the static config) so they still hold if the security
 * catalog overrides permissions — a mismatch is a real finding, not flake.
 * Regexes target unambiguous label/description text on the top-level nav button.
 */
