...
export default function CurriculumManagement() {
  ...
  // GRADE LEVEL OPTIONS FOR SUBJECTS & CURRICULA (as shown in dropdown screenshot)
  ...
  const handleDeleteCourse = (id: string) => {
    if (confirm("Are you sure you want to delete this course program? Any subjects associated with it might be unlinked.")) {
      deleteCourse(id);
    }
  };
  ...
  const handleDeleteSubject = (id: string) => {
    if (confirm("Are you sure you want to remove this subject from the catalog?")) {
      deleteSubject(id);
    }
  };
  ...
  const handleCreateCurriculum = (e: React.FormEvent) => {
    ...
    alert(`Success: ${generatedName} created!`);
  };
  ...
  const handleDeleteActiveCurriculum = () => {
    if (!activeCurriculum) return;
    if (confirm(`Are you absolutely sure you want to delete "${activeCurriculum.name}"?`)) {
